...
function timesTen(numbers) {
  const timesTen = numbers.map(number => number * 10);
  return timesTen;
}
...
function justAges(people) {
  const justAges = people.map(people => people.age);
  return justAges;
}
...
function onlyOver20(numbers) {
  const overTwenty = numbers.filter((number) => {
    return number > 20;
  });
  return overTwenty;
}
...
/*
  ...
  return a number that is the total amout of time spent on the activity specifed
   for the name paramete
